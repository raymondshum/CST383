...
#no-loop version
colrs =  df.am == 1 #manuals
colrs[colrs==1]='orange' #sewt to orange
colrs[colrs==0]='blue'
plt.scatter(df['disp'], df['mpg'], color=colrs, marker='^')
plt.xlabel('displacement (cu. inches)')
plt.ylabel('miles per gallon')
plt.title('MPG by engine displacement4')
plt.axhline(y=df['mpg'].mean(), color='black', linestyle='dotted')
...
df0 = df[df.am == 0]
df1 = df[df.am == 1]
plt.scatter(df0['disp'], df0['mpg'], color='blue', marker='^', label='automatic')
plt.scatter(df1['disp'], df1['mpg'], color='orange', marker='^', label='manual')
plt.xlabel('displacement (cu. inches)')
plt.ylabel('miles per gallon')
plt.title('MPG by engine displacement')
plt.axhline(y=df['mpg'].mean(), color='black', linestyle='dotted')
...
labels = cyl_counts.index
y_pos = np.arange(len(labels))
plt.bar(y_pos, cyl_counts, color='darkred')
plt.xticks(y_pos, labels)
